...
public class GlobalExceptionHandler {
    ...
    private int makeHttpStatus(BaseException ex) {
        HttpStatus status = Optional.ofNullable(ex.getData().httpStatus()).orElse(HttpStatus.BAD_REQUEST);
        return status.value();
    }
    ...
    private ResponseMessage<Void> makeResponse(BaseException ex, Locale locale) {
        return new ResponseMessageBuilder<Void>()
                .addSource(Optional.ofNullable(ex.getData().source()).orElse(getSource()))
    ...
    public ResponseEntity<ResponseMessage<Void>> handleValidation(ValidationException ex, Locale locale) {
        return ResponseEntity.status(makeHttpStatus(ex)).body(makeResponse(ex, locale));
    }

    @ExceptionHandler(SystemException.class)
    public ResponseEntity<ResponseMessage<Void>> handleValidation(SystemException ex, Locale locale) {
        return ResponseEntity.status(makeHttpStatus(ex)).body(makeResponse(ex, locale));
    }
    ...
    public ResponseMessage<Map<String, String>> handleInvalidArguments(MethodArgumentNotValidException ex) {
        return new ResponseMessageBuilder<Map<String, String>>()
                .addSource(getSource())
                .fail(ApiCodeConstant.GENERAL_ERR, DevErrorConstant.GENERAL_ERROR)
    ...
    public ResponseMessage<Void> handleException(Exception ex) {
        ...
        return new ResponseMessageBuilder<Void>()
                ...
                .fail(ApiCodeConstant.GENERAL_ERR, DevErrorConstant.GENERAL_ERROR)
                .addDevMessage(ex.getMessage())
                .addHttpStatus(HttpStatus.INTERNAL_SERVER_ERROR)
                .build();
    }
}
